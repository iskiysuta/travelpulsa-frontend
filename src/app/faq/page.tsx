    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <div className="space-y-4">
        {items.map((f: any) => {
          ...
          return (
            <details key={f.id} className="group bg-white rounded-lg shadow overflow-hidden hover:bg-primary/5">
              <summary className="cursor-pointer list-none px-6 py-4 flex items-center justify-between hover:text-primary">
                <span className="text-base md:text-lg font-semibold text-secondary group-hover:text-primary">{attrs.question}</span>
                <span className="ml-4 text-primary transition-transform duration-200 group-open:rotate-180">▾</span>
              </summary>
              <div className="px-6 pb-6 text-gray-600 prose max-w-none" dangerouslySetInnerHTML={{ __html: attrs.answer }} />
            </details>
          );
        })}
      </div>
